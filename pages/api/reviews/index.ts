...
async function reviewExists(user_id: string, collection_id: string): Promise<boolean> {
  try {
    const { data, error } = await supabaseClient
      .from('reviews')
      .select('id')
...
async function addReview(review: Review): Promise<Review | null> {
  try {
    ...
    const { data, error } = await supabaseClient.from('reviews').insert([review]).select('*');
